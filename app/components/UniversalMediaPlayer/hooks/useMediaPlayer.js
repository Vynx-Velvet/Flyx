...
export const useMediaPlayer = ({
  ...
  const playerActions = useMemo(() => ({
    ...
    seek: (time) => {
      const video = videoRef.current;
      if (!video || !isFinite(time)) return false;
    ...
    setVolume: (volume) => {
      const video = videoRef.current;
      if (!video || !isFinite(volume)) return false;
    ...
    setPlaybackRate: (rate) => {
      const video = videoRef.current;
      if (!video || !isFinite(rate)) return false;
  ...
  }, [forceStateSync, handleRecoverableError, onLoadedMetadata, onTimeUpdate, onPlay, onPause, onError, onLoadStart, onSeeking, onSeeked, onVolumeChange, onLoadProgress, onEnded]);
